...
class AllHM extends StatefulWidget {
  AllHM({this.id});
  int? id;
  @override
  State<AllHM> createState() => _AllHMState();
}

class _AllHMState extends State<AllHM> {
  final textnotifications = TextEditingController();

  @override
  void initState() {
    Future.delayed(Duration.zero).then((value) async {
      await homeworkProvider.allhmfromroom(widget.id);
    });
    super.initState();
  }
      ...
      body: Stack(
        children: [
          ...
          ListView(
              ...
              children: [
                homeworkProvider.isloadinggetallhomework
                    ? ListSkelatonList(
                        height: height,
                        width: width,
                      )
                    : ListView.builder(
                        physics: BouncingScrollPhysics(),
                        shrinkWrap: true,
                        itemCount: homeworkProvider.listhomework.length,
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        Text(
                                          "${AppLocalizations.of(context)!.idhomework}: ${homeworkProvider.listhomework[index].id}",
                                          style: TextStyle(
                                              fontSize: 18,
                                              fontWeight: FontWeight.bold),
                                        ),
                                        Text(
                                            "${homeworkProvider.listhomework[index].text}"),
                                        homeworkProvider.listhomework[index]
                                                    .image ==
                                                null
                                            ? Center(
                                                child: Image.asset(
                                            ...
                                            : Center(
                                                child: InstaImageViewer(
                                                  child: Image.network(
                                                    "${AppApi.IMAGEURL}${homeworkProvider.listhomework[index].image}",
                                                    width: width * 0.9,
                                                    fit: BoxFit.fitWidth,
                                                  ),
                                                ),
